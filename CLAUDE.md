...
**Wide panels** (`[wide]`):
```scss
// No blur/transform effects, just fade
filter: none;
transform: none;
```
...
```css
dropdown-panel {
  min-width: 300px; /* Override default 180px */
}
```
...
2. Check CSS specificity - component styles might need `!important` overrides
3. Verify CSS variables are set on `:root` or higher specificity selector
